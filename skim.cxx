 ...
 */
template <typename T> auto MinimalSelection(T &df) {
  return df
      .Filter("HLT_IsoMu17_eta2p1_LooseIsoPFTau20 == true", "Passes trigger")
 ...
 */
template <typename T> auto FindGoodMuons(T &df) {
  return df.Define(
      "goodMuons",
 ...
 *
 * The tau candidates in this collection represent hadronic decays of taus,
 * which means that the tau decays to combinations of pions and neutrinos in the
 * final state.
 */
template <typename T> auto FindGoodTaus(T &df) {
 ...
 */
template <typename T> auto FilterGoodEvents(T &df) {
 ...
 * a neutrino.
 */
template <typename T> auto FindMuonTauPair(T &df) {
  ...
  return df
      .Define("pairIdx", build_pair,
              {"goodMuons", "Muon_pt", "Muon_eta", "Muon_phi", "goodTaus",
               "Tau_relIso_all", "Tau_eta", "Tau_phi"})
 ...
 */
template <typename T> auto DeclareVariables(T &df) {
  ...
  return df.Define("pt_1", "Muon_pt[idx_1]")
      ...
      .Define("npv", "PV_npvs")
      .Define("goodJets",
              "Jet_puId == true && abs(Jet_eta) < 4.7 && Jet_pt > 30")
 ...
 */
template <typename T>
auto AddEventWeight(T &df, const std::string &sample, const float numEvents,
                    const float xsec, const float lumi, const float scale) {
  if (sample.find("Run2012") != std::string::npos) {
    return df.Define("weight", [=]() { return 1.0; });
  } else {
    return df.Define("weight",
                     [=]() { return xsec / numEvents * lumi * scale; });
  }
}
...
const std::vector<std::string> finalVariables = {
    "njets",   "npv",   "pt_1",   "eta_1",  "phi_1",  "m_1",     "iso_1",
    "q_1",     "mt_1",  "pt_2",   "eta_2",  "phi_2",  "m_2",     "iso_2",
    "q_2",     "mt_2",  "dm_2",   "jpt_1",  "jeta_1", "jphi_1",  "jm_1",
    "jbtag_1", "jpt_2", "jeta_2", "jphi_2", "jm_2",   "jbtag_2", "pt_met",
    "phi_met", "m_vis", "pt_vis", "mjj",    "ptjj",   "jdeta",   "gen_match",
    "run",     "weight"};
...
int main(int argc, char **argv) {
  if (argc != 6) {
    std::cout << "Use executable with following arguments: ./skim input output "
                 "cross_section integrated_luminosity scale"
              << std::endl;
    return -1;
  }
